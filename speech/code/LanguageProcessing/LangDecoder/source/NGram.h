...
namespace st {
    ...
    private:
        ...
        float                      _getStartEndProbability(const DataVector& input_sequence);
    };
    ...
    float BiGram<T>::getElemLogProbabilityWithHistory(const ElemType& elem, const DataVector& history)const
    {
        ...
        find_iter = m_elemIndexesMap.find(histoty.back());
        if (find_iter == m_elemIndexesMap.end()) {
            return ZERO_LOG_PROBABILITY;
        }
    ...
    {
        int nDistValues = m_elemIndexesMap.size() + _SYS_ELEMS_MAX;
    ...
    template<typename T>
    float BiGram<T>::_getStartEndProbability(const DataVector& input_sequence)
    {
        std::map<ElemType, int>::const_iterator find_iter;
